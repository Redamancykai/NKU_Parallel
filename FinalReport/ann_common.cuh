...
void PrintUsage(const char* program) {
    std::cerr
        ...
        << "[--mode baseline|topk|fp16|int8] [--hybrid] [--cpu-ratio R] [--queries N] [--batch N] "
        ...
        << "  " << program << " --data /anndata --index ivf --hybrid --nlist 1024 --nprobe 10 --batch 128\n";
}
...
Options ParseOptions(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--data") {
            ...
            options.hybrid = true;
        } else if (arg == "--cpu-ratio") {
            options.cpu_ratio = std::stod(need_value(arg));
            options.hybrid = true;
        } else if (arg == "--queries") {
    ...
    if (options.cpu_ratio < 0.0 || options.cpu_ratio > 1.0) {
        throw std::runtime_error("--cpu-ratio must be in [0, 1]");
    }
