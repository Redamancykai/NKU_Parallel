...
struct IVFHybridSearchStats {
    double probe_ms = 0.0;
    double resident_upload_ms = 0.0;
    double query_probe_upload_ms = 0.0;
...
std::vector<std::priority_queue<std::pair<float, uint32_t>>> CudaIVFFusedSearchBatch(
    ...
    size_t nprobe,
    double& elapsed_ms,
    IVFHybridSearchStats* stats = nullptr) {
    float* d_vectors = nullptr;
    float* d_queries = nullptr;
    float* d_top_distances = nullptr;
    uint32_t* d_ids = nullptr;
    ...
    try {
        if (stats != nullptr) {
            stats->resident_upload_ms = 0.0;
            stats->query_probe_upload_ms = 0.0;
            stats->gpu_batch_ms = 0.0;
            stats->end_to_end_ms = 0.0;
            stats->resident_bytes =
                index.vectors.size() * sizeof(float) +
                index.ids.size() * sizeof(uint32_t) +
                index.offsets.size() * sizeof(uint32_t) +
                index.sizes.size() * sizeof(uint32_t);
            stats->query_batches = (query_number + batch_size - 1) / batch_size;
        }

        auto resident_start = std::chrono::high_resolution_clock::now();
        CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_vectors),
                              index.vectors.size() * sizeof(float)));
        CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_ids),
                              index.ids.size() * sizeof(uint32_t)));
        ...
        CUDA_CHECK(cudaMemcpy(d_sizes,
                              ...
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaDeviceSynchronize());
        auto resident_end = std::chrono::high_resolution_clock::now();
        if (stats != nullptr) {
            stats->resident_upload_ms = std::chrono::duration<double, std::milli>(
        ...
        for (size_t first = 0; first < query_number; first += batch_size) {
            ...
            if (stats != nullptr) {
                stats->query_probe_upload_ms += std::chrono::duration<double, std::milli>(
        ...
        elapsed_ms = std::chrono::duration<double, std::milli>(end - start).count();
        if (stats != nullptr) {
            stats->gpu_batch_ms = elapsed_ms;
            stats->end_to_end_ms = stats->probe_ms +
                                   stats->resident_upload_ms +
                                   stats->gpu_batch_ms;
        }
    } catch (...) {
        cudaFree(d_vectors);
